...
std::string processQuery(const std::string& input) {
    ...
    if (!mysql_real_connect(&mysql, DB_HOST.c_str(), DB_USER.c_str(), DB_PASSWORD.c_str(), DB_NAME.c_str(), 0, NULL, 0)) {
    ...
    // 执行查询语句
    std::string sqlquery = "select FILE_PATH from " + TABLE_NAME + " where FILE_PATH like '%"+input+"%';";
